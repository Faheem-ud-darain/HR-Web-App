...
import { Users, Clock, CheckCircle2, ClipboardList, PlusCircle, Loader2, Trash2, Eye, AlertTriangle, Video, Forward } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { hrActions, Profile, Announcement, useProfiles, useLeaves, useTasks, useTeams, useAnnouncements, useWarehouses, useTimesheets, displayName, addEmployeeServer, setTeamLeadAdmin } from '@/lib/hrData';
...
export default function HRDashboard() {
  ...
  // use for their own trigger points. HR's own dashboard, so every
  // forward here notifies Admin (the other role).
      ...
      <Modal isOpen={isAnnounceOpen} onClose={() => setIsAnnounceOpen(false)} title="Create New Announcement">
        <form onSubmit={handleAnnouncementSubmit} className="space-y-4 pt-1">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
            <Button type="button" variant="outline" size="md" disabled={isPostingAnnouncement} onClick={() => setIsAnnounceOpen(false)}>Cancel</Button>
