...
export function GoogleIntegrationCard({ profile, onUpdate }: GoogleIntegrationCardProps) {
  ...
  // Fixed 2026-08-18 — this whole flow depends on window.open() returning a
  // real popup window that can postMessage back once Google's OAuth
  // callback page runs, and on Google actually allowing the OAuth screen to
  // load at all. Neither holds inside the native app's WKWebView (there's
  // no popup surface for window.open() to open into, and Google's own
  // policy blocks OAuth consent screens from loading inside any embedded/
  // in-app webview regardless). Tapping "Connect Google Account" there
  // silently did nothing — a real Guideline 2.1 "app completeness" risk if
  // an App Reviewer happens to try it. This can't be fixed by swapping in a
  // system-browser plugin alone either: completing the flow still needs a
  // way for that separate browser session to hand the result back to this
  // specific screen, which needs its own deep-link/custom-URL-scheme setup
  // (Info.plist + AppDelegate + a native-aware callback route) — a real
  // feature, not a one-line fix. Simplest correct fix for now: don't offer
  // a button that can't work; tell people to finish this from the web
  // dashboard instead. See isNativeMobileApp() usage below.
  const handleConnect = () => {
    setConnecting(true);
    setMsg(null);
      ...
      <div className="p-6 space-y-4">
        ...
        {!googleData ? (
          <div className="text-center py-4 space-y-3">
            ...
            {isNativeMobileApp() ? (
              // Can't complete Google's OAuth consent screen inside the
              // native app's webview (see handleConnect's comment above) —
              // show what's actually true instead of a button that does
              // nothing when tapped.
              <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-200 rounded-xl p-3.5 text-left max-w-sm mx-auto">
                <Smartphone className="h-4 w-4 text-amber-600 shrink-0 mt-0.5" />
                <p className="text-[11px] text-slate-700 font-semibold leading-relaxed">
                  Connecting a Google Account isn&apos;t available in the mobile app. Open the web dashboard on a desktop/laptop browser and connect it from your profile there — it&apos;ll apply to this app too.
                </p>
              </div>
            ) : (
              <button
